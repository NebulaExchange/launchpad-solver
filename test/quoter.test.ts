...
test('quote response uses raw values and formats intent correctly', async () => {
  ...
  const mockStateManager = {
    ...
    applyTradeDelta: jest.fn(),
    updateFromChain: jest.fn(),
    cacheQuoteResp: jest.fn(),
  } as unknown as import('../src/services/state-manager.service').StateManagerService;

  const service = new QuoterService(mockStateManager, mockNear);
  ...
  expect(quote?.quote_output.amount_out).toMatch(/^\d+$/);
